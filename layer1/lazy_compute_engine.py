...
class GraphNode:
    ...
    def __init__(self, 
        ...
        # Optimization hints
        self.is_deterministic = self.metadata.get('deterministic', True)
        self.is_expensive = self.metadata.get('expensive', False)
        self.can_parallelize = self.metadata.get('parallelizable', True)
...
class LazyComputeCapability(ComputeCapability[T]):
    ...
    def materialize(self) -> T:
        """Enhanced materialization with robust engine reference handling."""
        if self._is_materialized and self._materialized_result is not None:
            return self._materialized_result
        
        # ROBUST: Handle engine reference corruption
        engine = None
        if hasattr(self.engine, '__call__'):
            # Weak reference - dereference safely
            engine = self.engine()
        ...
        if engine is None:
            # FALLBACK: Direct execution without engine optimization
            warnings.warn("Engine unavailable, executing operation directly", RuntimeWarning)
            result = self._execute_node_directly(self.root_node)
        else:
            # Execute through engine for optimization
            try:
                result = engine._execute_graph(self.root_node, self.estimated_size)
            except AttributeError:
                # Engine missing _execute_graph method - use fallback
                result = self._execute_node_directly(self.root_node)
        
        # Cache deterministic results
        if getattr(self.root_node, 'is_deterministic', True):
            self._materialized_result = result
            self._is_materialized = True
        
        return result
        
    def _execute_node_directly(self, node) -> T:
        """Direct node execution with parameter optimization."""
        try:
            if not hasattr(node, 'inputs') or not node.inputs:
                # Source node - execute operation
                return node.operation()
            else:
                # Process dependencies first
                input_results = [self._execute_node_directly(inp) for inp in node.inputs]
                
                # OPTIMIZED: Intelligent parameter passing
                import inspect
                try:
                    sig = inspect.signature(node.operation)
                    param_count = len(sig.parameters)
                    
                    if param_count == 0:
                        return node.operation()
                    elif param_count == 1:
                        return node.operation(input_results[0] if input_results else None)
                    else:
                        return node.operation(*input_results[:param_count])
            ...
            warnings.warn(f"Node execution failed: {e}", RuntimeWarning)
            return pl.DataFrame()  # Safe fallback

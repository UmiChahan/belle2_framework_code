...
class Belle2Layer2Framework:
    ...
    def _init_variable_labels(self) -> Dict[str, str]:
        """Initialize comprehensive Belle II variable labels with units."""
        return {
            # Kinematic variables
            'pRecoil': r'$p_{\mathrm{recoil}}$ [GeV/$c$]',
            'eRecoil': r'$E_{\mathrm{recoil}}$ [GeV]',
            'mRecoil': r'$m_{\mathrm{recoil}}$ [GeV/$c^2$]',
            'm2Recoil': r'$m^2_{\mathrm{recoil}}$ [GeV$^2$/$c^4$]',
            
            # B meson variables
            'M_bc': r'$M_{\mathrm{bc}}$ [GeV/$c^2$]',
            'delta_E': r'$\Delta E$ [GeV]',
            'Mbc': r'$M_{\mathrm{bc}}$ [GeV/$c^2$]',  # Alternative
            'deltaE': r'$\Delta E$ [GeV]',  # Alternative
            
            # Particle kinematics
            'E': r'$E$ [GeV]',
            'p': r'$p$ [GeV/$c$]',
            'pt': r'$p_{\mathrm{T}}$ [GeV/$c$]',
            'px': r'$p_x$ [GeV/$c$]',
            'py': r'$p_y$ [GeV/$c$]',
            'pz': r'$p_z$ [GeV/$c$]',
            'theta': r'$\theta$ [rad]',
            ...
            # Angular variables
            'cosTheta': r'$\cos\theta$',
            'cosThetaCMS': r'$\cos\theta_{\mathrm{CMS}}$',
            'angular_separation': r'$\Delta\theta$ [rad]',
            'EoPRecoil': r'$E/p_{\mathrm{recoil}}$',
            
            # Detector variables
            'clusterE': r'$E_{\mathrm{cluster}}$ [GeV]',
            'clusterTiming': r'$t_{\mathrm{cluster}}$ [ns]',
            'nCDCHits': r'$N_{\mathrm{CDC hits}}$',
            'nSVDHits': r'$N_{\mathrm{SVD hits}}$',
            ...
            # PID variables
            'electronID': r'$\mathcal{L}_e$',
            'muonID': r'$\mathcal{L}_\mu$',
            'pionID': r'$\mathcal{L}_\pi$',
            'kaonID': r'$\mathcal{L}_K$',
            'protonID': r'$\mathcal{L}_p$',
            'absdPhi': r'$|\Delta\phi(\mu_1,\mu_2)|$ [rad]',
            'absdPhiMu1': r'$|\Delta\phi(\mu_1,\mathrm{recoil})|$ [rad]',
            'absdPhiMu2': r'$|\Delta\phi(\mu_2,\mathrm{recoil})|$ [rad]',
            'absdThetaMu1': r'$|\Delta\theta(\mu_1,\mathrm{recoil})|$ [rad]',
            'absdThetaMu2': r'$|\Delta\theta(\mu_2,\mathrm{recoil})|$ [rad]',
            'absdAlpha1': r'$|\alpha(\mu_1,\mathrm{recoil})|$ [rad]',
            'absdAlpha2': r'$|\alpha(\mu_2,\mathrm{recoil})|$ [rad]',
            'dRMu1': r'$\Delta R(\mu_1,\mathrm{recoil})$',
            'dRMu2': r'$\Delta R(\mu_2,\mathrm{recoil})$',
            'pTRecoil': r'$p_{\mathrm{T}}^{\mathrm{recoil}}$ [GeV/$c$]',
            'mu1Pt': r'$p_{\mathrm{T}}^{\mu_1}$ [GeV/$c$]',
            'mu2Pt': r'$p_{\mathrm{T}}^{\mu_2}$ [GeV/$c$]',
            'deltaMu1PRecoil': r'$\Delta(\mu_1,\mathrm{recoil})$ [rad]',
            'deltaMu2PRecoil': r'$\Delta(\mu_2,\mathrm{recoil})$ [rad]',
            'deltaMu1ClusterPRecoil': r'$\Delta(\mu_1^{\mathrm{cluster}},\mathrm{recoil})$ [rad]',
            'deltaMu2ClusterPRecoil': r'$\Delta(\mu_2^{\mathrm{cluster}},\mathrm{recoil})$ [rad]',
            'min_deltaMuPRecoil': r'$\min\Delta(\mu,\mathrm{recoil})$ [rad]',
            'min_deltaMuClusterPRecoil': r'$\min\Delta(\mu^{\mathrm{cluster}},\mathrm{recoil})$ [rad]',
                    }

    def set_variable_label(self, variable: str, label: str):
        """Set custom label for a variable."""
        self._custom_labels[variable] = label

    def get_variable_label(self, variable: str) -> str:
        """Get LaTeX label for variable with fallback."""
        # Check custom labels first
        if variable in self._custom_labels:
            return self._custom_labels[variable]
        
        # Check standard labels
        if variable in self._variable_labels:
            return self._variable_labels[variable]
        
        # Intelligent fallback with formatting
        return self._format_variable_fallback(variable)

    def _format_variable_fallback(self, variable: str) -> str:
        """Create formatted fallback for unknown variables."""
        ...
        if parts[0] in ['n', 'N']:
            ...
            formatted = 'N'
            if len(parts) > 1:
                formatted += f"_{{{' '.join(parts[1:])}}}"
            return f"${formatted}$"
        
        # Default: replace underscores with subscripts
        if len(parts) > 1:
            main = parts[0]
...
def quick_analysis(data_path: str,
    ...
    df_best = framework.keep_one_candidate(df_cut)
